package dev.footer.general_silicon.menu;
...
public class RefineryMenu extends AbstractContainerMenu {
    ...
    public RefineryMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(MenuInit.REFINERY_MENU.get(), id);
        checkContainerSize(inv, 3);
        entity = be = (RefineryBE) entity;
        this.data = data;
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, be.getBlockPos()),
                player, Blocks.REFINERY.get());
    }

    private void addPlayerInv(Inventory pInv) {
        for(int i = 0; i < 3; i++) {
            for(int l = 0; l < 9; l++) {
                this.addSlot(new Slot(pInv, l + i * 9 + 9, 8 + l * 18, 86 + i * 18));
            }
        }
    }

    private void addPlayerHotbar(Inventory pHotbar) {
        for(int i = 0; i < 9; i++) {
            this.addSlot(new Slot(pHotbar, i, 8 + i * 18, 144));
        }
    }
}
